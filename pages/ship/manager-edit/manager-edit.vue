	...
	<view class="page" :style="{'background-color': active === 0 ? '' : '#fff'}">
		...
		<view class="search" v-if="active === 0">
            ...
            <view class="search__info">
                ...
                新的管理员必须现在该平台完成实名认证，企业管理与拥有最高权限请务必确认信息！
            </view>
            <view class="form">
                <view class="formInput">
                    <view class="formInput__left">
        ...
        <view class="foot">
            ...
            <button v-else :disabled="valueCode.length === 0" class="foot__btn" type="primary" @tap="_sendBtn()">确认更换船舶管理员</button>
        </view>
	</view>
</template>
...
<script>
    ...
    import { getShipSearchStaff, postShipManagerCode, postShipManagerConfirm } from '@/service/getData.js';
	...
	export default {
        ...
        methods: {
            ...
            sendCode() {
                ...
                toast.loading('获取验证码...');
                postShipManagerCode(this.ship_id).then(res => {
                    toast.hideLoading();
                    this.message = '';
...
// this.valueCode = '123456';
            ...
            _sendBtn() {
                toast.loading('发送验证码...');
                postShipManagerConfirm({ship_id: this.ship_id, user_id: this.data.id, phone: this.valuePhone, sec_key: this.secCodekey, sec_code: this.valueCode}).then(res => {
                    toast.hideLoading();
                    this.PAGE_TEMP_ADD({field: 'shipManagerEdit', shipManagerEdit: true});
                    uni.showModal({
                        title: '更换船舶管理员',
                        content: '提交成功!',
                        showCancel: false,
