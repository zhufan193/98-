	...
	<view class="page">
        <template v-if="companyArray.length < 2">
            <view class="empty">
                ...
                <text class="empty__text">您当前没有认证的船舶代理类企业，</text>
                <text class="empty__text" style="padding-top: 0;">只有拥有认证的船舶代理类企业才能发布。</text>
            </view>
            <view class="foot">
...
<script>
    ...
    import { getShipAgentDetail, uploadPic, postService, postVerifyCencel } from '@/service/getData.js';
	...
	export default {
        ...
        onLoad(options) {
            ...
            if(options && options.id){
                ...
                this.is_edit = true;
                getCompanyFinancialDetail().then(res => {
                    toast.hideLoading();
                    console.log(res)
                }, err => {
                    console.log(err)
        ...
        methods:{
            ...mapActions(['getProvince', 'getCompanyShipAgent']),
            _goPage(url) {
                router.navigateTo(url);
            },
            async _getSelect() {
                if(!this.companyTempData.hasOwnProperty('company_agent') || !this.companyTempData.company_agent.length){
                    await this.getCompanyShipAgent();
                }
                this.companyArray = this.companyArray.concat(this.companyTempData.company_agent);
