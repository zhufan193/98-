	...
	<view class="page">
        <template v-if="companyArray.length < 2">
            <view class="empty">
                ...
                <text class="empty__text">您当前没有认证的船舶备件类企业，</text>
                <text class="empty__text" style="padding-top: 0;">只有拥有认证的船舶备件类企业才能发布。</text>
            </view>
            <view class="foot">
        ...
        <template v-else>
            <view class="card card--full form__list">
                ...
                <view class="form__cell" @tap="_showDrawer('helper')">
                    <view class="form__label">提供备件</view>
            ...
            <uni-drawer :visible="drawerVisible" mode="right" @close="_hideDrawer">
                <view class="drawer">
                    <scroll-view class="drawer__scroll" scroll-y :style="{height: height + 'px'}">
                        <wlp-collapse title="提供备件" mode="multi-select" type="component" :options="componentArray" :open="true" @change="_changeDrawer"></wlp-collapse>
                    </scroll-view>
...
<script>
	...
	export default {
        ...
        methods:{
            ...mapActions(['getCompanyComponent', 'getComponentType']),
            _goPage(url) {
                router.navigateTo(url);
            },
            async _getSelect() {
                if(!this.companyTempData.hasOwnProperty('company_component') || !this.companyTempData.company_component.length){
                    await this.getCompanyComponent();
                }
                this.companyArray = this.companyArray.concat(this.companyTempData.company_component);
                
                await this.getComponentType();
                this.componentArray = this.componentArray.concat(this.dictionaryTempData.component_type);
            ...
            _clearDrawer(){
                for(let i = 0, len = this.componentArray.length; i < len; i++){
                    if(this.componentArray[i].checked){
                        this.$set(this.componentArray[i], 'checked', false);
                    }
                }
                this.data.type_ids = '';
                this.data.type = '';
            ...
            _sendDrawer(){
                this.data.type_ids = this.componentArray.filter(n => n.checked === true).map(item => {
                    return item.id
                }).join(',');
                this.data.type = this.componentArray.filter(n => n.checked === true).map(item => {
                    return item.name
                }).join(',');
                this._hideDrawer();
            },
            _changeDrawer(e){
                let obj = this.componentArray[e.value];
                if(obj.checked){
