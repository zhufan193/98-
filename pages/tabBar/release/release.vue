<template>
	<view class="page">
        <!-- #ifndef APP-PLUS -->
        <!-- 小程序端显示/app端页面在pages/common/release -->
        <view class="full">
            <view class="time">
                {{time}}
            </view>
...
<script>
	...
	export default {
        ...
        // 手机端回退跳转
        onShow() {
            let index = localStorage.getStore('tabBarIndex');
            if(index != 2){
                tabBarIndex = index;
                router.navigateTo('/pages/common/release/release');
            }else{
                let tabBarItem = ['/pages/tabBar/home/home','/pages/tabBar/sail/sail','','/pages/tabBar/pallet/pallet','/pages/tabBar/profile/profile'];
                router.switchTab(tabBarItem[tabBarIndex]);
            }
        },
        // #endif
        // #ifndef APP-PLUS
...
<style lang="scss">
    ...
    .page{
        ...
        padding: 0 $uni-spacing-row-lg * 2 0;
