	...
	<view class="page">
        <template v-if="companyArray.length < 2">
            <view class="empty">
                ...
                <text class="empty__text">您当前没有认证的燃油类企业，</text>
                <text class="empty__text" style="padding-top: 0;">只有拥有认证的燃油类企业才能发布。</text>
            </view>
            <view class="foot">
        ...
        <template v-else>
            <view class="card card--full form__list">
                ...
                <view class="form__cell" @tap="_showDrawer()">
                    <view class="form__label">服务港口</view>
                    <view :class="data.service_harbor ? 'form__field' : 'form__note'">
                        {{data.service_harbor ? data.service_harbor : '点击选择'}}
                    </view>
                </view>
            </view>
                        ...
                        <view id="select" class="select">
                            ...
                            <view class="select__main">
                                ...
                                <block v-for="(item, index) of selectData" :key="index">
                                    <view class="select__btn" @click="_portRemove(index)">
                                        {{item.port_name}}
                                    </view>
                                </block>
                            </view>
                        </view>
                        <wlp-port :height="height - selectHeight" @click="_portChange"></wlp-port>
                    </scroll-view>
...
<script>
    ...
    import { getCompanyFinancialDetail, uploadPic, postOilStore } from '@/service/getData.js';
	...
	export default {
        ...
        methods:{
            ...mapActions(['getGlobal', 'getCompanyFuel']),
            _goPage(url) {
            ...
            _sendDrawer(){
                this.data.service_harbor_ids = this.selectData.map(item => {
                    return item.port_id
                }).join(',');
                this.data.service_harbor = this.selectData.map(item => {
            ...
            _clearDrawer(){
                this.selectData = [];
                this.data.service_harbor_ids = '';
                this.data.service_harbor = '';
            },
