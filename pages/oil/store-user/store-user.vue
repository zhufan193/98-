<template>
	<view class="page">
        <block v-for="(item, index) of data" :key="index">
            <wlp-card-oil-store :options="item" :index="index" :show="item.show" @click="_goDetail"></wlp-card-oil-store>
        </block>
...
<script>
    ...
    import { getOilStoreList } from '@/service/getData.js';
	...
	export default {
        ...
        methods: {
            ...
            _goDetail(param) {
                ...
                router.navigateTo('/pages/oil/store-detail/store-detail', {id: this.data[param.index].id, router: 'user'});
            },
            getData(type='DownRefresh') {
                ...
                getOilStoreList({filter: 'owner', page: this.page}).then(res => {
                    if(this.page == 1){
                        this.data = res.data;
                    }else{
