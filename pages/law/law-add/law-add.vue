    ...
    <view class="page">
        <template v-if="companyArray.length < 2">
            <view class="empty">
                ...
                <text class="empty__text">您当前没有认证的法律咨询类企业，</text>
                <text class="empty__text" style="padding-top: 0;">只有拥有认证的法律咨询类企业才能发布。</text>
            </view>
            <view class="foot">
        ...
        <template v-else>
            <view class="card card--full form__list">
                ...
                <view class="form__cell" @tap="_showDrawer('city')">
                    <view class="form__label">服务区域</view>
                    <view :class="data.service_city ? 'form__field' : 'form__note'">
                        {{data.service_city ? data.service_city : '点击选择'}}
                    </view>
                </view>
            </view>
            ...
            <uni-drawer :visible="drawerVisible" mode="right" @close="_hideDrawer">
                <view class="drawer">
                    <scroll-view class="drawer__scroll" scroll-y :style="{height: height + 'px'}">
                        <wlp-collapse title="省份" mode="multi-select" type="province" :options="drawerProvince" :open="true" @change="_changeDrawer"></wlp-collapse>
                    </scroll-view>
...
<script>
    ...
    import { getCompanyLawDetail, uploadPic, postService, postVerifyCencel } from '@/service/getData.js';
	...
	export default {
        ...
        computed: mapState(['userInfo','companyTempData']),
        onLoad(options) {
        ...
        methods:{
            ...mapActions(['getProvince', 'getCompanyLegalAdvice']),
            _goPage(url) {
                router.navigateTo(url);
            },
            async _getSelect() {
                if(!this.companyTempData.hasOwnProperty('company_legal') || !this.companyTempData.company_legal.length){
                    await this.getCompanyLegalAdvice();
                }
                this.companyArray = this.companyArray.concat(this.companyTempData.company_legal);
                
                await this.getProvince();
                this.drawerProvince = JSON.parse(localStorage.getStore('region_province'));
            ...
            _clearDrawer(){
                for(let i = 0, len = this.drawerProvince.length; i < len; i++){
                    if(this.drawerProvince[i].checked){
                        this.$set(this.drawerProvince[i], 'checked', false);
                    }
                }
                this.data.service_city_ids = '';
                this.data.service_city = '';
            },
            _sendDrawer(){
                this.data.service_city_ids = this.drawerProvince.filter(n => n.checked === true).map(item => {
                    return item.id
                }).join(',');
                this.data.service_city = this.drawerProvince.filter(n => n.checked === true).map(item => {
                    return item.name
                }).join(',');
                this._hideDrawer();
            },
            _changeDrawer(e){
                let obj = this.drawerProvince[e.value];
                if(obj.checked){
