<template>
	<view class="page">
        ...
        <wlp-fab title="发布财务咨询" @click="_release()"></wlp-fab>
	</view>
</template>
...
<script>
    ...
    import { getCompanyFinancialList } from '@/service/getData.js';
	...
	export default {
        ...
        onShow() {
            if(this.screenTemp.hasOwnProperty('financial') && this.screenTemp.financial.init){
                let screen = this.screenTemp.financial;
        ...
        onNavigationBarButtonTap() { // 标题栏按钮点击事件
            router.navigateTo('/pages/financial/financial-screen/financial-screen');
        },
        methods: {
            ...
            _release() {
                ...
                router.navigateTo('/pages/financial/financial-add/financial-add');
            },
            _goDetail(param) {
                router.navigateTo('/pages/company/detail/detail', {type: 'finance_advice', id: this.data[param.index].id});
            },
            getData(type = 'DownRefresh') {
                ...
                getCompanyFinancialList(send).then(res => {
                    if(this.page == 1){
                        this.data = res.data;
                    }else{
